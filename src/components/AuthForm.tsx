...
const AuthForm: React.FC<AuthFormProps> = ({ onSignIn, onSignUp, onBack }) => {
  ...
  const handleStripeCheckout = async () => {
    ...
    try {
      ...
      setSuccess('Ansluter till Stripe...');
      ...
      if (!url) {
        setError('Kunde inte initiera betalning. Vänligen försök igen.');
        return;
      }
      ...
      let errorMessage = 'Betalningsfel. Kontrollera din internetanslutning och försök igen.';
      ...
      if (err instanceof TypeError && err.message.includes('fetch')) {
        errorMessage = 'Anslutningsfel. Kontrollera din internetanslutning och försök igen.';
      }
              ...
              <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-lg p-4 border border-green-200 mb-4">
                ...
                <div className="space-y-2">
                  <p className="text-xs sm:text-sm text-green-700 font-semibold">
                    💰 {mainCourse?.name}: {coursePrice}
                  </p>
                  <p className="text-xs text-green-600">
                    Engångsbetalning - Livstidsåtkomst
                  </p>
                </div>
